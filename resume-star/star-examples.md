...
### 长度要求
- 每条 Bullet Point **不超过 80 个汉字**（约 2 行），这是一个硬约束
- 如果超过 80 字，必须拆分或压缩——优先保留"怎么做的"和"效果"
- 不要为了简洁而丢失技术细节，但也不能写成一个段落

### STAR 展开与 Bullet Point 的差异化要求
- **Bullet Point** 是极致压缩版：只保留最核心的技术方案 + 量化结果，删掉背景和问题叙述
- **STAR 展开** 是面试回忆版：重点放在**为什么这样做**（技术选型理由、遇到的困难、设计权衡），而不是重复 BP 已经说过的"怎么做的"
- 两者重叠率不应超过 40%——如果 STAR 展开只是在复述 BP 的内容，说明展开不够深
